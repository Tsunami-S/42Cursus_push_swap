...
/*   Updated: 2025/01/03 11:26:09 by tssaito          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ash(t_list *list, t_act act)
{
	...
	select_act(list, act);
	if((prev == pa && act == pa) || (prev == pb && act == pb) || (prev == ra && act == ra) || (prev == rb && act == rb) || (prev == rra && act == rra) || (prev == rrb && act == rrb) )
	{
		if(!i)
			i = 2;
		else
	...
	if(i && ((prev == pa && act == pb )||( prev == pb && act == pa) ||( prev == ra && act == rra )|| (prev == rb && act == rrb)||( prev == rra && act == ra )|| (prev == rrb && act == rb)))
	{
		i--;
		if(!i)
			prev = none;
	...
	else if(i > 0)
	{
		...
		write_command(rrr);
	else if(!((prev == pa && act == pb )||( prev == pb && act == pa) ||( prev == ra && act == rra )|| (prev == rb && act == rrb)||( prev == rra && act == ra )|| (prev == rrb && act == rb)))
	{
		write_command(prev);
		prev = act;
		return;
